...
<script type="text/javascript">
          ...
          jQuery.ajax({
				   ...
				   success: function(data){
					   ...
					   if(data.statusCode == 200){
						  ...
						  <#if table.attachmentFlag?if_exists && table.attachmentFlag == "1">
	...
	function removeById(filename){
          ...
          jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/dataFile/deleteByName?serviceKey=${serviceKey}&businessKey_enc=${businessKey_enc}&status_enc=${status_enc}',
				   dataType: 'json',
				   data: params,
				   error: function(data){
					   alert('服务器处理错误！');
	...
	function downById(fileId){
		window.open("${contextPath}/dataFile/download?fileId="+fileId);
	}
</script>
</head>
<body>
...
jQuery(function () {
    ...
    var url = '${contextPath}/dataFile/doUpload?tableId=${tableId}&businessKey_enc=${businessKey_enc}';
    ...
    jQuery('#fileupload').fileupload({
        url: url,
        dataType: 'json',
		<#if table.attachmentExts?if_exists && table.attachmentExts != "">
		acceptFileTypes: /(\.|\/)(${table.attachmentExts})$/i,
		</#if>
		<#if table.attachmentFlag?if_exists && table.attachmentFlag == "1">
		maxNumberOfFiles: 1,
		</#if>
		<#if table.attachmentSize?if_exists && table.attachmentSize &gt; 0 >
