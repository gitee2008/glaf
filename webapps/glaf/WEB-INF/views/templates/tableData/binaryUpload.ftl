...
<script type="text/javascript">
    ...
    function deleteById(fileId){
		if(confirm("文件删除后不能恢复，确定删除吗？")){
          jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/matrix/binary/deleteById?fileId='+fileId,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function removeById(filename){
          ...
          jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/matrix/binary/deleteByName?serviceKey=${serviceKey}&businessKey_enc=${businessKey_enc}&status_enc=${status_enc}',
				   dataType: 'json',
				   data: params,
				   error: function(data){
					   alert('服务器处理错误！');
	...
	function downById(fileId){
		window.open("${contextPath}/matrix/binary/download?fileId="+fileId);
	}
</script>
</head>
<body>
...
jQuery(function () {
    ...
    var url = '${contextPath}/matrix/binary/doUpload?serviceKey=${serviceKey}&businessKey_enc=${businessKey_enc}';
    ...
    jQuery('#fileupload').fileupload({
        url: url,
        dataType: 'json',
		acceptFileTypes: /(\.|\/)(jpg|jpeg|png|gif|bmp|flv|swf|mkv|avi|rm|rmvb|mpeg|mpg|ogg|ogv|mov|wmv|mp4|webm|mp3|wav|mid|rar|zip|tar|gz|7z|bz2|cab|iso|doc|docx|xls|xlsx|ppt|pptx|pdf|txt|md|xml)$/i,
		maxNumberOfFiles: 10,
		maxFileSize: 999000,
