...
<script type="text/javascript">
     ...
     $(function(){
		...
		var setting = {
			async: {
				enable: true,
				url:"${contextPath}/flowable/tree/json",
		...
		function myTreeOnClick(event, treeId, treeNode, clickFlag) {
			 ...
			 if(actionType == '1'){
				 if(treeNode.nlevel==0){
                    url = '${contextPath}/flowable/tree/processInstances?processDefinitionKey='+treeNode.key;
				 } else if(treeNode.nlevel==1){
                    url = '${contextPath}/flowable/tree/processInstances?processDefinitionId='+treeNode.id;
				 }
                
			 } else if(actionType == '2'){
				 if(treeNode.nlevel==0){
                   url = '${contextPath}/flowable/tree/historyProcessInstances?processDefinitionKey='+treeNode.id;
				 } else if(treeNode.nlevel==1){
                   url = '${contextPath}/flowable/tree/historyProcessInstances?processDefinitionId='+treeNode.id;
				 }
			 }
			 //alert(url);
			 loadData(url);
    ...
    function onMyDbClickRow(rowIndex, row){
       window.open('${contextPath}/flowable/task?processInstanceId='+row.id+"&processDefinitionId="+row.processDefinitionId);
	 }
...
<style type="text/css">
.ztree li span.button.process_folder_ico_open{margin-right:2px; background: url(${contextPath}/static/images/orm_root.gif) no-repeat scroll 0 0 transparent; vertical-align:top; *vertical-align:middle}
.ztree li span.button.process_folder_ico_close{margin-right:2px; background: url(${contextPath}/static/images/orm.gif) no-repeat scroll 0 0 transparent; vertical-align:top; *vertical-align:middle}
.ztree li span.button.process_leaf_ico_docu{margin-right:2px; background: url(${contextPath}/static/images/pkg.gif) no-repeat scroll 0 0 transparent; vertical-align:top; *vertical-align:middle}
</style>
</head>
<body>  
