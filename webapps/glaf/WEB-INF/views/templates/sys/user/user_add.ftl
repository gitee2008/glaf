...
<script language="javascript">
    ...
    function saveData(){
		if(jQuery("#userId").val()==""){
			alert("用户账户不能为空");
            document.getElementById("userId").foucs();
			return;
		}
		if(jQuery("#name").val()==""){
			alert("用户姓名不能为空");
		...
		jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/sys/user/saveAdd',
				   data: params,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
  ...
  <tr>
	...
	<table width="95%" align="center" border="0" cellspacing="0" cellpadding="5">
      ...
      <tr>
        ...
        <td width="80%">
		<input id="userId" name="userId" type="text" size="30" class="x-text" datatype="string" nullable="no" maxsize="10" chname="用户名">
		</td>
      </tr>
      <tr>
