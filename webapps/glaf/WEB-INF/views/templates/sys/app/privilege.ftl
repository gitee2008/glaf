...
<script type="text/javascript">
	...
	function getTreeUrl(){
		return "${contextPath}/sys/application/permissionTreeJson?parentId=${parentId}&tenantId=${tenant.tenantId}&type=${type}";
	}
	...
	function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/sys/treePermission/saveTenantTreePermission?tenantId=${tenant.tenantId}&type=${type}&privilege=${privilege}',
				   dataType: 'json',
				   data: params,
				   error: function(data){
					   alert('服务器处理错误！');
...
<div class="toolbar-backgroud" style="height:42px"> 
    <img src="${contextPath}/static/images/window.png">&nbsp;<span class="x_content_title">设置租户【${tenant.name}】的模块权限</span>
    &nbsp;&nbsp;
