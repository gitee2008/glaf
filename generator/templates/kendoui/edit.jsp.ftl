...
<title>�༭${tableDefinition.title}��Ϣ</title>
...
<script type="text/javascript">
  ...
  <#if pojo_fields?exists>
   ...
   function save(){
	   ...
	   //form.method="post";
	   //form.action = "<%=request.getContextPath()%>/${tableDefinition.moduleName}/${modelName}/save${entityName}";
	   //form.submit();
	   var link = "<%=request.getContextPath()%>/${tableDefinition.moduleName}/${modelName}/save${entityName}";
...
<div class="x_content_title"><img
	src="<%=request.getContextPath()%>/images/window.png" alt="�༭${tableDefinition.title}��Ϣ">&nbsp;
�༭${tableDefinition.title}��Ϣ</div>
<br>
...
<table width="95%" align="center" border="0" cellspacing="0" cellpadding="5">
 <#if pojo_fields?exists>
  ...
  <tr>
	...
	<input id="${field.name}" name="${field.name}" type="text" class="k-textbox" 
	  <#if field.nullable == false> required="true"  </#if> data-bind="value: ${field.name}"
	       value="#F{${modelName}.${field.name}}" validationMessage="������${field.title}"/>
  ...
  <!-- <tr>
	  ...
	  <label class="k-radio-label" for="engine1">��Ч</label>&nbsp;&nbsp;
	  <input type="radio" name="status" id="engine2" class="k-radio" value="1" >
	  <label class="k-radio-label" for="engine2">��Ч</label>
	</td>
    </tr> -->
    ...
    <tr>
        ...
        <div>
          <button type="button" id="iconButton"  class="k-button k-primary" style="width: 90px" onclick="javascript:save();">����</button>
	</div>
	</td>
      </tr>
</table>   
