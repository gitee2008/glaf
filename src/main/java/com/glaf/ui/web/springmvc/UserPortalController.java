...
public class UserPortalController {
	protected static final Log logger = LogFactory.getLog(UserPortalController.class);
	...
	@RequestMapping
	public ModelAndView myPortal(HttpServletRequest request, ModelMap modelMap) {
		...
		UserPanel userPanel = panelService.getUserPanelByDashboardId(loginContext.getActorId(), dashboardId);
		if (userPanel == null) {
		...
		List<UserPortal> userPortals = userPortalService.getUserPortalsByDashboardId(loginContext.getActorId(),
				dashboardId);
		if (userPortals == null || userPortals.isEmpty()) {
			...
			if (userPortals == null || userPortals.isEmpty()) {
				...
				if (panels != null && !panels.isEmpty()) {
					...
					userPortals = userPortalService.getUserPortalsByDashboardId(loginContext.getActorId(), dashboardId);
				}
			}
		}
		...
		for (UserPortal p : userPortals) {
			...
			panel.setPanelButtons(panelButtonService.getPanelButtonByParentId(panel.getId()));
			p.setPanel(panel);
		}
