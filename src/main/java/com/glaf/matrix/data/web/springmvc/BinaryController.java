...
@Controller("/matrix/binary")
@RequestMapping("/matrix/binary")
public class BinaryController {
	protected final static Log logger = LogFactory.getLog(BinaryController.class);
			...
			try {
				...
				for (Entry<String, MultipartFile> entry : entrySet) {
					...
					if (mFile.getOriginalFilename() != null && mFile.getSize() > 0
							&& mFile.getSize() <= FileUtils.MB_SIZE * 50) {
	...
	public ModelAndView binarylist(HttpServletRequest request, ModelMap modelMap) {
		LoginContext loginContext = RequestUtils.getLoginContext(request);
