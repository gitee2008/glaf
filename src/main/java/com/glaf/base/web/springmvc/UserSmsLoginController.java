...
public class UserSmsLoginController {
	...
	protected static final Log logger = LogFactory.getLog(UserSmsLoginController.class);
	...
	public String getNowYearMonthDayHHmm() {
	...
	public byte[] sendSms(HttpServletRequest request) {
		String mobile = request.getParameter("mobile");
				...
				if (num >= 130 && num <= 199) {
					...
					if (user != null && user.getLocked() == 0) {
						...
						if (smsVerifyMessageService.getSmsCount(mobile, dateAfter) <= 10) {
							String id = mobile + "_" + getNowYearMonthDayHHmm();
							...
							if (smsVerifyMessage == null) {
								...
								smsVerifyMessage.setName(user.getName());
