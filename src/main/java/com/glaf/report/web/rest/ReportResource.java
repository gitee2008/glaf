...
@Controller("/rs/report")
@Path("/rs/report")
public class ReportResource {
	protected static final Log logger = LogFactory.getLog(ReportResource.class);
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_OCTET_STREAM })
	public byte[] deleteAll(@PathParam("reportIds") String rowIds, @Context HttpServletRequest request)
			throws IOException {
		if (rowIds != null) {
			List<String> ids = StringTools.split(rowIds);
			if (ids != null && !ids.isEmpty()) {
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_OCTET_STREAM })
	public byte[] deleteById(@PathParam("reportId") String reportId, @Context HttpServletRequest request)
			throws IOException {
		reportService.deleteById(reportId);
		return ResponseUtils.responseJsonResult(true);
	}
	...
	@javax.annotation.Resource
	public void setTableDefinitionService(ITableService tableService) {
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_OCTET_STREAM })
	public byte[] view(@PathParam("reportId") String reportId, @Context HttpServletRequest request) throws IOException {
		Report report = null;
		if (StringUtils.isNotEmpty(reportId)) {
			report = reportService.getReport(reportId);
		}
