...
@Controller("/rs/reportFile")
@Path("/rs/reportFile")
public class ReportFileResource {
	protected static final Log logger = LogFactory.getLog(ReportFileResource.class);
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_OCTET_STREAM })
	public byte[] deleteById(@PathParam("reportFileId") String reportFileId, @Context UriInfo uriInfo) {
		if (reportFileId != null) {
			reportFileService.deleteById(reportFileId);
			return ResponseUtils.responseJsonResult(true, "删除成功！");
		} else {
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_OCTET_STREAM })
	public byte[] saveReportFile(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
		String reportFileId = request.getParameter("reportFileId");
		if (StringUtils.isEmpty(reportFileId)) {
			reportFileId = request.getParameter("id");
		}
