...
@Controller("/rs/reportTask")
@Path("/rs/reportTask")
public class ReportTaskResource {
	protected static final Log logger = LogFactory.getLog(ReportTaskResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_OCTET_STREAM })
	public byte[] deleteAll(@PathParam("rowIds") String rowIds, @Context UriInfo uriInfo) {
		if (rowIds != null) {
			List<String> ids = StringTools.split(rowIds);
			if (ids != null && !ids.isEmpty()) {
				// reportTaskService.deleteByIds(ids);
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_OCTET_STREAM })
	public byte[] deleteById(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_OCTET_STREAM })
	public byte[] deleteById(@PathParam("reportTaskId") String reportTaskId, @Context UriInfo uriInfo) {
		if (reportTaskId != null) {
			reportTaskService.deleteById(reportTaskId);
			return ResponseUtils.responseJsonResult(true, "删除成功！");
		} else {
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_OCTET_STREAM })
	public byte[] saveReportTask(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
		String reportTaskId = request.getParameter("reportTaskId");
		if (StringUtils.isEmpty(reportTaskId)) {
			reportTaskId = request.getParameter("id");
		}
