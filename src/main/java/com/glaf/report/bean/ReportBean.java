...
public class ReportBean {
	...
	public Map<String, Object> populate(String reportId, String actorId, Map<String, Object> context) {
		Map<String, Object> dataMap = new HashMap<String, Object>();
		ReportDefinition reportDefinition = ReportContainer.getContainer().getReportDefinition(reportId);
		if (reportDefinition != null) {
			logger.debug(reportDefinition.getReportId() + " -> " + reportDefinition.getTemplateFile());
						...
						if (rowSetList != null && !rowSetList.isEmpty()) {
							for (ReportRowSet rs : rowSetList) {
								...
								if ("mybatis3".equals(dataMgr)) {
									...
									String dataMgrClassName = rs.getDataMgrClassName();
									if (StringUtils.isNotEmpty(dataMgrClassName)) {
										dataManager = (DataManager) ClassUtils.instantiateObject(dataMgrClassName);

									}
									String dataMgrBeanId = rs.getDataMgrBeanId();
									if (StringUtils.isNotEmpty(dataMgrBeanId)) {
										Object bean = ContextFactory.getBean(dataMgrBeanId);
								...
								if (dataManager != null) {
									if (rptMgrMapping != null) {
										...
										String clazz = dataManager.getClass().getSimpleName();
									...
									if ("mybatis3".equals(dataMgr)) {
										if (StringUtils.isNotEmpty(parameterType)) {
											...
											rows = mybatis.getResultList(connection, query, parameter);
										} else {
											rows = dataManager.getResultList(connection, query, context);
										}
									} else {
										rows = dataManager.getResultList(connection, query, context);
									}
									...
									if (rows != null && rows.size() > 0) {
										if (singleResult) {
											...
											dataMap.put("dataModel_" + mapping, object);
										} else {
											dataMap.put(mapping, rows);
											dataMap.put("dataModel_" + mapping, rows);
										}
									}
								}
							}
