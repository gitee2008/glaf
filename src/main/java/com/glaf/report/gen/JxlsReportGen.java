...
public class JxlsReportGen extends AbstractReportGen implements ReportGen {
	...
	public byte[] createReport(Report report, Connection connection,
		...
		try {
			...
			destWorkbook = transformer.transformXLS(bis, params);
			if (StringUtils.isNotEmpty(report.getPostProcessor())) {
				List<String> processors = StringTools.split(report
						.getPostProcessor());
				if (processors != null && !processors.isEmpty()) {
					for (String processorClass : processors) {
						try {
							Object processor = ClassUtils
									.instantiateObject(processorClass);
							if (processor instanceof XlsProcessor) {
								XlsProcessor p = (XlsProcessor) processor;
								p.process(template, destWorkbook);
							}
						} catch (Exception ex) {
							ex.printStackTrace();
						}
