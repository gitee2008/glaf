...
public class MergeXlsProcessor implements XlsProcessor {
	protected static final Log logger = LogFactory
			.getLog(MergeXlsProcessor.class);
	...
	protected JSONObject getCurrentJsonData() {
		if (jsonDataThreadLocal.get() != null
				&& !jsonDataThreadLocal.get().isEmpty()) {
	...
	protected String getCurrentXmlTag() {
		if (xmlTagThreadLocal.get() != null
				&& !xmlTagThreadLocal.get().isEmpty()) {
	 ...
	 *            Excel目标文件
	 */
	public void process(Workbook template, Workbook workbook) {
					...
					for (int colIndex = 0; colIndex < colCount; colIndex++) {
						...
						if (isBegin) {
							...
							for (int k = rowIndex + 1; k < rowCount; k++) {
								boolean isEnd = this.isEndRow(sheet, k,
										colIndex);
								if (isEnd) {
									// 合并单元格
									CellRangeAddress newMergedRegion = new CellRangeAddress(
											rowIndex + 1, k - 1, colIndex,
											colIndex);
					...
					if (isMerge) {
						...
						if (json != null) {
							...
							if (json.containsKey("hiddenBeginRow")) {
								int hiddenBeginRow = json
										.getIntValue("hiddenBeginRow");
							...
							if (json.containsKey("hiddenEndRow")) {
								int hiddenEndRow = json
										.getIntValue("hiddenEndRow");
	...
	protected void setCellValue(Sheet sheet, int rowIndex, int colIndex,
			String value) {
		Row row = sheet.getRow(rowIndex);
		if (row != null) {
			Cell cell = row.getCell(colIndex);
