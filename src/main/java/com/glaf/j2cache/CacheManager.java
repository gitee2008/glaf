...
public class CacheManager {
	...
	private final static CacheProvider getProviderInstance(String value) throws Exception {
		...
		if ("geode".equalsIgnoreCase(value)) {
			className = "com.glaf.j2cache.geode.GeodeCacheProvider";
		}
	...
	public static void initCacheProvider(CacheExpiredListener listener) {
		...
		try {
			...
			String j2cacheLevel1 = SystemConfig.getValueByKey("j2cacheLevel1Provider");
			if (StringUtils.isNotEmpty(j2cacheLevel1)) {
				log.info("Using L1 Cache:" + j2cacheLevel1);
				l1_provider = getProviderInstance(j2cacheLevel1);
			} else {
				l1_provider = getProviderInstance(props.getProperty("cache.L1.provider_class"));
			}
			l1_provider.start(getProviderProperties(props, CacheManager.l1_provider));
			log.info("Using L1 CacheProvider : " + l1_provider.getClass().getName());

			String j2cacheLevel2 = SystemConfig.getValueByKey("j2cacheLevel2Provider");
			if (StringUtils.isNotEmpty(j2cacheLevel2)) {
				log.info("Using L2 Cache:" + j2cacheLevel2);
				l2_provider = getProviderInstance(j2cacheLevel2);
			} else {
				l2_provider = getProviderInstance(props.getProperty("cache.L2.provider_class"));
			}

			if (l2_provider != null) {
				l2_provider.start(getProviderProperties(props, CacheManager.l2_provider));
			...
			throw new CacheException("Unabled to initialize cache providers", ex);
		} finally {
