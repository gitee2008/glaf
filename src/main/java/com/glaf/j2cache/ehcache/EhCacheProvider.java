...
public class EhCacheProvider implements CacheProvider {
	 ...
	 */
	public synchronized EhCache buildCache(String name, boolean autoCreate, CacheExpiredListener listener)
			throws CacheException {
	...
	public void start(Properties props) throws CacheException {
		...
		if (props != null) {
			ehcacheName = (String) props.get(KEY_EHCACHE_NAME);
		}
		if (ehcacheName != null && ehcacheName.trim().length() > 0) {
			cacheManager = CacheManager.getCacheManager(ehcacheName);
		}
		if (cacheManager == null) {
			// 指定了配置文件路径,加载之
			if (props.containsKey(KEY_EHCACHE_CONFIG_XML)) {
				cacheManager = new CacheManager(props.getProperty(KEY_EHCACHE_CONFIG_XML));
			} else {
				// 加载默认实例
		...
		if (cacheManager == null) {
			...
			if (props != null) {
				configurationResourceName = (String) props.get(EHCACHE_CONFIGURATION_RESOURCE_NAME);
			}
			...
			String filename = SystemProperties.getConfigRootPath() + configurationResourceName;
