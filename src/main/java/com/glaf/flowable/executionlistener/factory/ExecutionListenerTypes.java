...
public class ExecutionListenerTypes {
	...
	private static final Log logger = LogFactory.getLog(ExecutionListenerTypes.class);

	static Map<String, Class<?>> executionListeners = initializeExecutionListenerTypes();

	private final static String DEFAULT_CONFIG = "com/glaf/flowable/executionlistener/factory/flowable.executionlistener.xml";
	...
	static Map<String, Class<?>> initializeExecutionListenerTypes() {
		...
		String resource = SystemProperties.getString("flowable.executionListeners");
		if (StringUtils.isEmpty(resource)) {
			resource = DEFAULT_CONFIG;
		}
		if (StringUtils.isNotEmpty(resource)) {
			InputStream actionTypesStream = PropertiesUtils.getInputStream(resource);
			Element executionListenersElement = XmlUtils.parseXmlInputStream(actionTypesStream).getDocumentElement();
			Iterator<?> nodeTypeIterator = XmlUtils.elementIterator(executionListenersElement, "executionListeners");
		...
		String ext_resource = CustomProperties.getString("flowable.executionListeners");
