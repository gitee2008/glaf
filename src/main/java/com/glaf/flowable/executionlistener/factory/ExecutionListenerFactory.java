...
public class ExecutionListenerFactory {
	...
	private static KeyedObjectPoolFactory poolFactory = new StackKeyedObjectPoolFactory(factory, 50, 5);
	private static KeyedObjectPool pool = poolFactory.createPool();
	// 自定义ExecutionListener实现类执行调用类型，取值为spring或pool
	private final static String EXECUTION_LISTENER_FACTORY_TYPE = "flowableExecutionListenerFactoryType";
	...
	public static void notify(String key, DelegateExecution execution) {
		...
		if (StringUtils.isNotEmpty(SystemProperties.getString(EXECUTION_LISTENER_FACTORY_TYPE))) {
			executionListenerType = SystemProperties.getString(EXECUTION_LISTENER_FACTORY_TYPE);
		}
		if (StringUtils.isNotEmpty(CustomProperties.getString(EXECUTION_LISTENER_FACTORY_TYPE))) {
			executionListenerType = CustomProperties.getString(EXECUTION_LISTENER_FACTORY_TYPE);
		}
		...
		if (StringUtils.equals(executionListenerType, "spring")) {
			executionListener = (ExecutionListener) ExecutionListenerBeanFactory.getBean(key);
