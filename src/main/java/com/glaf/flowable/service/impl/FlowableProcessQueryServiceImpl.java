...
import com.glaf.flowable.model.UserTask;
import com.glaf.flowable.service.FlowableProcessQueryService;
import com.glaf.flowable.xml.BpmnXmlReader;

@Service("flowableProcessQueryService")
@Transactional(readOnly = true)
public class FlowableProcessQueryServiceImpl implements FlowableProcessQueryService {
	protected static final Log logger = LogFactory.getLog(FlowableProcessQueryService.class);
	...
	public ProcessDefinitionInfo getLatestProcessDefinitionByKey(String processDefinitionKey) {
		...
		if (processDefinitionImpl != null) {
			...
			if (inputStream != null) {
				...
				List<ActivityInfo> flowablees = reader.read(root, processDefinitionKey);
				processDefinitionInfo.setFlowablees(flowablees);
	...
	public ProcessInstanceInfo getProcessInstanceInfo(String processInstanceId) {
		...
		if (processInstance != null) {
			...
			List<ActivityInfo> flowablees = pdi.getFlowablees();

			if (flowablees != null && !flowablees.isEmpty()) {
				logger.debug("flowablees:" + flowablees.size());
				Map<String, ActivityInfo> actMap = new java.util.HashMap<String, ActivityInfo>();
				for (ActivityInfo act : flowablees) {
